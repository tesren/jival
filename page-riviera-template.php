<?php
    /*
    Template Name: Riviera Maya Template
    */
    get_header();

    $developments = get_posts([
        ...
        <?php if ( $developments ): ?>
            ...
            <div class="container row justify-content-center mb-6">
                ...
                <?php foreach( $developments as $dev ): ?>
                    ...
                    <?php $images = rwmb_meta('gallery', ['limit' =>1 , 'size' => 'medium_large' ], $dev->ID ) ?>

                    <div class="col-12 col-lg-6">
                        <a href="<?= get_the_permalink($dev->ID) ?>" class="card text-decoration-none text-blue rounded-0 shadow-4">
                            <img src="<?= $images[0]['url'] ?>" class="w-100" alt="<?= get_the_title($dev->ID) ?>" style="height:370px; object-fit:cover;">
                            <div class="p-4">
                                <h2 class="card-title fw-normal"><?= get_the_title($dev->ID) ?></h2>
                                <p class="card-text fw-light fs-5 mb-1">
                                    <i class="fa-solid fa-location-dot"></i> <?php get_list_terms($dev->ID, 'regiones') ?>
                                </p>
                                <p class="fs-4 mb-1"><?php pll_e('Precios desde') ?>: $<?= number_format($dev->price) ?> <?= $dev->currency ?></p>
                            </div>
                        </a>
                    </div>
